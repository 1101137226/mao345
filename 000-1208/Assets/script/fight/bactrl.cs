...
public class bactrl : MonoBehaviour
{
	...
	void Start ()
	{
		...
		GameObject.Find ("low(Clone)").transform.localPosition = new Vector3 (-8000, -1300, 0);
		GameObject.Find ("elow(Clone)").transform.parent = father;
		GameObject.Find ("elow(Clone)").transform.localPosition = new Vector3 (-8000, 913, 0);
		isspc = false;
	...
	void Update ()
	{
		...
		if (!pause.stop) {
			if (Input.GetKeyDown (KeyCode.K) && changeflag == true) {
				...
				GameObject.Find ("high(Clone)").transform.localPosition = new Vector3 (-4000, -1300, 0);
				sonicflag = true;
			...
			if (jkctrloff) {
				if (enjk == true && enchjk == true) {
					...
					GameObject.Find ("ehigh(Clone)").transform.localPosition = new Vector3 (-4000, 913, 0);
