...
public class NETBS : NetworkBehaviour {
	...
	string target;
	bool changeflag;
	[SyncVar(hook="jksync")]public bool jk=false;
	Transform father;

	bool down;
	float time;
	[SyncVar(hook="combosync")]public short combo=0;
	[SyncVar(hook="decksync")]public short deckleft=0;
	...
	[SyncVar(hook="dessync")]public bool isdes=false;
	[SyncVar(hook="SyncStop")]public bool Stop=false;
	GameObject emeny;
	[SyncVar(hook="SyncDrop")]GameObject drop;
	...
	public void Usecard(string num)
	{
		...
		case 37:
		case 38:
			guup=1;smup=1;baup=1;piup=1;allup=1;
	...
	if (!jork) {
			...
			if (who == "gu") {
				...
				CmdAttack(dmg);CmdBassAtk(false);CmdPiano(false,dmg);CmdHeal(false,smup*allup*combodamageup*pos*weakup,rec);
			}else if(who=="duo")
			{
	...
	else if(jork)
		{
			if (who == "gu") {
				...
				CmdAttack(dmg);CmdBassAtk(true);CmdPiano(true,dmg);CmdHeal(true,(smup*allup+powerup)*combodamageup*pos*weakup,rec);
			} 
	...
	public void Draw()
	{
		...
		for (int q=0; q<5; q++) {
			...
			if (ishand [q] == false && handd.Count > 0) {
				...
				GameObject.Find (handd.Peek () + "(Clone)").transform.localPosition = new Vector3 (-6600+q*1800,-4000+(team-1)*10000,0);
				if(team==1)
	...
	void GetCardinDeck(byte t)
	{
		...
		for (int q=0; q<5; q++) {
			...
			GameObject.Find(handd.Peek()+"(Clone)").transform.parent=GameObject.Find("UIRoot").transform;
			GameObject.Find(handd.Peek()+"(Clone)").transform.localScale=new Vector3(1,1,1);
			GameObject.Find(handd.Peek()+"(Clone)").transform.localPosition=new Vector3(-6600+q*1800,-2800+(t-1)*6000,0);
			...
			}else if(!isLocalPlayer)
			{
				...
				GameObject.Find("cardback(Clone)").transform.localPosition=new Vector3(-6600+q*1800,-2800+(t-1)*6000,0);
